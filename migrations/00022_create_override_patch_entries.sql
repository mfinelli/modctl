...
CREATE TABLE override_patch_entries (
  ...
  CHECK (
    (patch_type IN ('ini_set', 'json_set', 'yaml_set', 'xml_set') AND entry_value IS NOT NULL)
    OR
    (patch_type IN ('ini_unset', 'json_unset', 'yaml_unset', 'xml_clear', 'xml_unset') AND entry_value IS NULL)
  ),
